...
LOGO_PATH    = LOGOS_DIR / "logo fondo transparente grande.png"
...
def generar_imagen_perla(perla: dict) -> str:
    ...
    # ── 5. Logo en esquina inferior derecha ───────────────────────────────
    if LOGO_PATH.exists():
        try:
            logo = Image.open(LOGO_PATH).convert("RGBA")
            logo_size = 150
            ratio_logo = logo_size / max(logo.width, logo.height)
            logo = logo.resize(
                (int(logo.width * ratio_logo), int(logo.height * ratio_logo)),
                Image.LANCZOS
            )
            pos_logo_x = W - logo.width - 40
            pos_logo_y = H - logo.height - 50
...
if __name__ == "__main__":
    perla_test = {
        ...
        "contexto": "Temporada 2025-26 | @dos_aros"
    }
